...
namespace TTB
{
	...
	public class frmBiendong : System.Windows.Forms.Form
	{
		...
		private void InitializeComponent()
		{
            ...
            this.butIn.Location = new System.Drawing.Point(179, 218);
            ...
            this.butKetthuc.Location = new System.Drawing.Point(241, 218);
			...
			for(int i=y1;i<=y2;i++)
			{
				...
				for(int j=itu;j<=iden;j++)
				{
					...
					if (ttb.bMmyy(mmyy))
					{
                        ...
                        sql += " and a.ngay between to_date('" + tu.Text + "'," + stime + ") and to_date('" + den.Text + "',"+stime+")";
						sql+=" order by a.ngay";
						foreach(DataRow r in ttb.get_data(sql).Tables[0].Select("dongia>0","ten,ngay"))
						{
							...
							r1=ttb.getrowbyid(dstyle.Tables[0],"mabd="+int.Parse(r["mabd"].ToString()));
							if (r1==null)
							{
		...
		private void exp_excel()
		{
			try
			{
				...
				tenfile=ttb.Export_Excel(ds,"biendong");
				...
				osheet.get_Range(ttb.getIndex(0)+"1",ttb.getIndex(0)+"1").EntireColumn.Delete(Missing.Value);
				osheet.get_Range(ttb.getIndex(be)+dong.ToString(),ttb.getIndex(socot)+sodong.ToString()).NumberFormat="#,##0.00";
				osheet.get_Range(ttb.getIndex(0)+"4",ttb.getIndex(socot-1)+dongke.ToString()).Borders.LineStyle=XlBorderWeight.xlHairline;
