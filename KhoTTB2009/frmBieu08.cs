...
namespace TTB
{
	...
	/// </summary>
	public class frmBieu08 : System.Windows.Forms.Form
	{
		...
		private string s_userid,sql,mmyy,user;
		private int i_userid,i_col,i_row=0,i_nhom;
		...
		public frmBieu08(string hoten,int userid,string _mmyy,int nhom)
		{
			//
			// Required for Windows Form Designer support
			//
			InitializeComponent();
            s_userid = hoten; i_userid = userid; mmyy = _mmyy; i_nhom = nhom; this.Text += "\\" + s_userid;
			//
		...
		{
            System.ComponentModel.ComponentResourceManager resources = new System.ComponentModel.ComponentResourceManager(typeof(frmBieu08));
            ...
            this.butMoi.Location = new System.Drawing.Point(183, 500);
            ...
            this.butSua.Location = new System.Drawing.Point(244, 500);
            ...
            this.butLuu.Location = new System.Drawing.Point(305, 500);
            ...
            this.butBoqua.Location = new System.Drawing.Point(366, 500);
            ...
            this.butKetthuc.Location = new System.Drawing.Point(549, 500);
            ...
            this.butHuy.Location = new System.Drawing.Point(427, 500);
            ...
            this.butIn.Location = new System.Drawing.Point(488, 500);
            ...
            this.Text = "Trang thiết bị y tế";
            this.WindowState = System.Windows.Forms.FormWindowState.Maximized;
            this.Load += new System.EventHandler(this.frmBieu08_Load);
		...
		private void frmBieu08_Load(object sender, System.EventArgs e)
		{
            user = ttb.user;
			bPhatsinh=ttb.bSolieu;
			cmbNgay.DisplayMember="NGAY";
			cmbNgay.ValueMember="ID";
			load_ngay();
			dt=ttb.get_data("select * from "+user+".dm_08_dk").Tables[0];
			load_grid(false);
		...
		private void load_data()
		{
			if (bPhatsinh)
			{
                sql = "select a.*,b.stt,b.ten from " + user + ".bieu_08 a," + user + ".dm_08 b where a.ma=b.ma and a.id=" + l_id + " order by a.ma";
				ds=ttb.get_data(sql);
			}
			else
			{
		...
		{
            ds = ttb.get_data("select * from " + user + ".dm_08 order by ma");
			DataRow[] dr;
            foreach (DataRow r in ttb.get_data("select * from " + user + ".bieu_08 where id=" + l_id).Tables[0].Rows)
			{
		...
		{
            cmbNgay.DataSource = ttb.get_data("select ID,to_char(ngay,'dd/mm/yyyy') as NGAY from " + user + ".bieu_08 group by id,ngay order by ngay").Tables[0];
		}
		...
		private void butLuu_Click(object sender, System.EventArgs e)
		{
			try
			{
				if (ds.HasChanges())
				{
                    ttb.execute_data("delete from " + user + ".bieu_08 where id=" + l_id);
					...
					{
						ttb.upd_bieu08(l_id,int.Parse(r[i]["ma"].ToString()),ngay.Text,int.Parse(r[i]["c01"].ToString()),
		...
		private void dataGrid1_CurrentCellChanged(object sender, System.EventArgs e)
		{
			...
			DataRow r1=ttb.getrowbyid(dt,"ma='"+"C"+i_col.ToString().PadLeft(2,'0')+"'");
			if (r1!=null)
			{
			...
			try
			{
				...
				{
					MessageBox.Show("Số liệu không hợp lệ !",ttb.Msg);
					dataGrid1[i_row,i_col]=0;
				}
				if (dataGrid1[i_row,i_col].ToString()!="")
				{
					if (decimal.Parse(dataGrid1[i_row,i_col].ToString())>decimal.Parse(dataGrid1[i_row,3].ToString()) || (decimal.Parse(dataGrid1[i_row,4].ToString())+decimal.Parse(dataGrid1[i_row,5].ToString())+decimal.Parse(dataGrid1[i_row,6].ToString()))>decimal.Parse(dataGrid1[i_row,3].ToString()))
					{
						MessageBox.Show("Số liệu không hợp lệ !",ttb.Msg);
						dataGrid1[i_row,i_col]=0;
					}
		...
		private void get_data()
		{
            ...
            ds = ttb.get_data("select * from " + user + ".dm_08 order by ma");
			sql="select b.nhombo,";
			sql+="sum(a.tondau+a.slnhap-a.slxuat) as c01,";
			sql+="sum(a.tondau+a.slnhap-a.slxuat) as c02,";
			sql+="sum(0) as c03,";
			sql+="sum(0) as c04,";
			sql+="sum(case when d.loaibc=0 then a.tondau+a.slnhap-a.slxuat else 0 end) as c05,";
			sql+="sum(case when d.loaibc=1 then a.tondau+a.slnhap-a.slxuat else 0 end) as c06,";
			sql+="sum(case when d.loaibc=2 then a.tondau+a.slnhap-a.slxuat else 0 end) as c07";
			sql+=" from "+xxx+".ttb_tonkhoct a,"+user+".ttb_dmbd b,"+user+".ttb_dmnhom c,"+user+".ttb_dmnguon d,"+xxx+".ttb_theodoi t";
            sql+=" where a.stt=t.id and a.mabd=b.id and b.manhom=c.id and t.manguon=d.id";
			sql+=" and b.bhyt<>0 and c.theodoi=1 and b.nhombo is not null and b.nhom="+i_nhom;
			sql+="  and a.mmyy='"+mmyy+"' group by b.nhombo";
			sql+=" union all ";
			sql+="select b.nhombo,";
			sql+="sum(a.tondau+a.slnhap-a.slxuat) as c01,";
			sql+="sum(a.tondau+a.slnhap-a.slxuat) as c02,";
			...
			sql+="sum(case when d.loaibc=1 then a.tondau+a.slnhap-a.slxuat else 0 end) as c06,";
			sql+="sum(case when d.loaibc=2 then a.tondau+a.slnhap-a.slxuat else 0 end) as c07";
			sql+=" from "+xxx+".ttb_tutrucct a,"+user+".ttb_dmbd b,"+user+".ttb_dmnhom c,"+user+".ttb_dmnguon d,"+xxx+".ttb_theodoi t ";
            sql+=" where a.stt=t.id and a.mabd=b.id and b.manhom=c.id and t.manguon=d.id";
			sql+=" and b.bhyt<>0 and c.theodoi=1 and b.nhombo is not null and b.nhom="+i_nhom;
            sql += "   and a.mmyy='" + mmyy + "' group by b.nhombo";
			DataRow [] dr;
			...
			{
				dr=ds.Tables[0].Select("ma="+int.Parse(r["nhombo"].ToString()));
				if (dr.Length>0)
				{
