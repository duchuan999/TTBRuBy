...
namespace TTB
{
	...
	/// </summary>
	public class frmButruc : System.Windows.Forms.Form
	{
		...
		private string s_ngay,sql,s_makp,s_mmyy,s_kho,format_soluong,format_dongia,format_sotien;
		private DataTable dtdmbd=new DataTable();
		private DataTable dtkp=new DataTable();
		private DataTable dtll=new DataTable();
		private DataTable dtct=new DataTable();
		private DataTable dtphieu=new DataTable();
		private bool bKhoaso,bGiaban,bCongdon;
		...
		public frmButruc(AccessData acc,int nhom,string ngay,int loai,int userid,string users,string makp,string mmyy,string kho,bool giaban,bool congdon)
		{
			//
			...
			s_makp=makp;
			s_mmyy=mmyy;
			s_kho=kho;bGiaban=giaban;bCongdon=congdon;
			this.Text="Bù trực thuốc, vật tư y tế theo người bệnh ("+users.Trim()+")";
			//
		...
		{
			System.Resources.ResourceManager resources = new System.Resources.ResourceManager(typeof(frmButruc));
			...
			this.mabn.Location = new System.Drawing.Point(48, 8);
			this.mabn.Name = "mabn";
			...
			// label5
			// 
			this.label5.Location = new System.Drawing.Point(598, 8);
			...
			this.ngay.Location = new System.Drawing.Point(384, 8);
			this.ngay.Name = "ngay";
			...
			this.phieu.Location = new System.Drawing.Point(492, 8);
			...
			this.makp.Location = new System.Drawing.Point(640, 8);
			this.makp.Name = "makp";
			...
			this.butKetthuc.Location = new System.Drawing.Point(452, 486);
			...
			this.butDuyet.Location = new System.Drawing.Point(152, 486);
			...
			this.butThuhoi.Location = new System.Drawing.Point(227, 486);
			...
			this.butIn.Location = new System.Drawing.Point(302, 486);
			...
			this.butTim.Location = new System.Drawing.Point(377, 486);
			...
			this.Load += new System.EventHandler(this.frmButruc_Load);
		...
		{
			sql="select a.stt,a.sttt,'' doituong,a.mabd,b.ma,trim(b.ten)||' '||b.hamluong ten,b.tenhc,b.dang,f.ten tenkho,c.ten tennguon,d.ten tennhomcc,a.handung,a.losx,a.soluong,round(a.sotien/a.soluong,3) dongia,a.sotien,a.giaban,3 madoituong,a.makho,a.manguon,a.nhomcc,'xx/xx/xxxx' ngay ";
		...
		{
			sql="select c.makhoa makp,b.mabd,sum(b.slyeucau-b.slthuc) soluong,c.loai,c.phieu, f.ten tenphieu ";
			sql+=" from ttb_dutrull a,ttb_dutruct b,ttb_duyet c,ttb_dmbd d, ttb_dmphieu e, ttb_loaiphieu f ";
			sql+=" where a.id=b.id and a.idduyet=c.id and b.mabd=d.id and c.loai=e.id and c.phieu=f.id ";
			sql+=" and c.nhom="+i_nhom+" and c.loai=1";
			sql+=" and to_char(c.ngay,'dd/mm/yyyy')='"+s_ngay+"'";
			if (s_kho!="") sql+=" and b.makho in ("+s_kho.Substring(0,s_kho.Length-1)+")";
			sql+=" and c.done<>0 and b.slyeucau>b.slthuc and b.tutruc=1 group by c.makhoa,b.mabd,c.loai,c.phieu, f.ten ";
			sql+=" union all select c.makhoa makp,b.mabd,sum(b.slyeucau-b.slthuc) soluong ,c.loai,c.phieu, f.ten tenphieu ";
			sql+=" from ttb_xtutrucll a,ttb_xtutrucct b,ttb_duyet c,ttb_dmbd d, ttb_dmphieu e, ttb_loaiphieu f ";
			sql+=" where a.id=b.id and a.idduyet=c.id and b.mabd=d.id and c.loai=e.id and c.phieu=f.id ";
			sql+=" and c.nhom="+i_nhom+" and c.loai="+i_loai;
			sql+=" and to_char(c.ngay,'dd/mm/yyyy')='"+s_ngay+"'";
			if (s_kho!="") sql+=" and b.makho in ("+s_kho.Substring(0,s_kho.Length-1)+")";
			sql+=" and b.idvpkhoa=0 and c.done<>0 and b.slyeucau>b.slthuc group by c.makhoa,b.mabd,c.loai,c.phieu, f.ten ";
			...
			i_makp=0;
			string s_makp="";
			dr1=dtngay.Select("soluong>0","makp,loai,phieu,mabd");
			for(int j=0;j<dr1.Length;j++)
			{
				if (s_makp!=dr1[j]["makp"].ToString()+dr1[j]["loai"].ToString()+dr1[j]["phieu"].ToString())
				{
					i_makp=int.Parse(dr1[j]["makp"].ToString());
					s_makp=dr1[j]["makp"].ToString()+dr1[j]["loai"].ToString()+dr1[j]["phieu"].ToString();
					r=ttb.getrowbyid(dtkp,"id="+i_makp);
					if (r!=null)
					{
						node=treeView1.Nodes.Add(r["ten"].ToString()+" - "+dr1[j]["tenphieu"].ToString());
						dr=dtngay.Select("makp="+i_makp+" and loai="+dr1[j]["loai"].ToString()+" and phieu="+dr1[j]["phieu"].ToString());
...
//				return;
//			}
//			if (MessageBox.Show("Đồng ý hủy người bệnh \n"+hoten.Text,d.Msg,MessageBoxButtons.YesNo,MessageBoxIcon.Question)==DialogResult.Yes)
//			{
...
//			}
		...
		private void butThuhoi_Click(object sender, System.EventArgs e)
		{
			...
			frmThuhoi f=new frmThuhoi(ttb,dtll,dtct,dtkp,dtphieu,int.Parse(makp.SelectedValue.ToString()),int.Parse(phieu.SelectedValue.ToString()),s_ngay,s_mmyy,i_nhom,i_loai,false,false,1,s_kho,i_userid);
			f.ShowDialog(this);
			if (f.bOk)
			{
				dtll.AcceptChanges();
		...
		{
			frmInphieu f=new frmInphieu(ttb,dtkp,dtphieu,(makp.SelectedIndex==-1)?-1:int.Parse(makp.SelectedValue.ToString()),(phieu.SelectedIndex==-1)?-1:int.Parse(phieu.SelectedValue.ToString()),s_ngay,s_mmyy,i_nhom,i_loai,s_kho,"PHIẾU BÙ",false);
			f.ShowDialog(this);
		}
		...
		{
			l_id=0;
			frmTimbutruc f=new frmTimbutruc(this);
			f.MdiParent=parent;
			f.RefreshText("");
			f.Show();
		}
