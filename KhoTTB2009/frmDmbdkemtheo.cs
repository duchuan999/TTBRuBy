...
namespace TTB
{
	...
	public class frmDmbdkemtheo : System.Windows.Forms.Form
	{
		...
		public frmDmbdkemtheo(AccessData acc,int nhom,decimal id,string title)
		{
			//
		...
		{
			System.Resources.ResourceManager resources = new System.Resources.ResourceManager(typeof(frmDmbdkemtheo));
		...
		{
			dsdm.Clear();
			dsdm.Merge(ttb.get_data("select a.id,a.ma,trim(a.ten)||' '||a.hamluong ten,a.dang,a.tenhc,b.ten tenhang from ttb_dmbd a,ttb_dmhang b where a.mahang=b.id and a.id<>"+i_id+" and a.nhom="+i_nhom+" and a.id not in (select mabd from ttb_dmbdkemtheo where id="+i_id+") order by a.ten"));
		...
		private void butLuu_Click(object sender, System.EventArgs e)
		{
			...
			{
				foreach(DataRow r in dsdm.Tables[0].Select("chon=true")) ttb.upd_dmbdkemtheo(i_id,decimal.Parse(r["id"].ToString()),1);
				foreach (DataRow row in dsdm.Tables[0].Rows) row["chon"]=false;
				load_chitiet();
			}
			else foreach(DataRow r in dt.Rows) ttb.upd_dmbdkemtheo(i_id,decimal.Parse(r["mabd"].ToString()),decimal.Parse(r["soluong"].ToString()));
		}
		...
		private void butXoa_Click(object sender, System.EventArgs e)
		{
			try
			{
				...
				ttb.execute_data("delete from ttb_dmbdkemtheo where id="+i_id+" and mabd="+int.Parse(dataGrid2[i,0].ToString()));
				load_chitiet();
			}
		...
		{
			dt=ttb.get_data("select b.mabd,a.ma,trim(a.ten)||' '||a.hamluong ten,a.dang,c.ten tenhang,b.soluong from ttb_dmbd a,ttb_dmbdkemtheo b,ttb_dmhang c where a.id=b.mabd and a.mahang=c.id and b.id="+i_id+" order by a.ten").Tables[0];
