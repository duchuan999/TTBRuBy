...
namespace TTB
{
	...
	public class frmDmloai : System.Windows.Forms.Form
	{
		...
		private void InitializeComponent()
		{
            ...
            this.ten.Name = "ten";
            this.ten.Size = new System.Drawing.Size(345, 21);
            this.ten.TabIndex = 1;
            ...
            // label4
            // 
            this.label4.Location = new System.Drawing.Point(489, 460);
            ...
            this.nhomcc.Location = new System.Drawing.Point(537, 460);
		...
		private void frmDmloai_Load(object sender, System.EventArgs e)
		{
            ...
            user = ttb.user; itable = ttb.tableid("", "ttb_dmloai");
			nhomcc.DisplayMember="TEN";
			nhomcc.ValueMember="ID";
			nhomcc.DataSource=ttb.get_data("select * from "+user+".ttb_dmnhom where nhom="+i_nhom+" order by stt").Tables[0];
			load_grid();
			AddGridTableStyle();
			ref_text();
		}
		...
		{
            dt = ttb.get_data("select a.*,b.ten as tennhom from " + user + ".ttb_dmloai a left join " + user + ".ttb_dmnhom b on a.loai=b.id where a.nhom=" + i_nhom + " order by a.stt").Tables[0];
		...
		private void butMoi_Click(object sender, System.EventArgs e)
		{
			try
			{
				stt.Value=decimal.Parse(ttb.get_data("select max(stt) from "+user+".ttb_dmloai where nhom="+i_nhom).Tables[0].Rows[0][0].ToString())+1;
			}
			catch{stt.Value=1;}
			l_id=0;
			ten.Text="";
		...
		private void butLuu_Click(object sender, System.EventArgs e)
		{
            ...
            if (l_id == 0)
            {
                try
                {
                    l_id = decimal.Parse(ttb.get_data("select max(id) from " + user + ".ttb_dmloai").Tables[0].Rows[0][0].ToString()) + 1;
                }
                catch { l_id = 1; }
            ...
            else
            {
                ...
                ttb.upd_eve_upd_del(itable, i_userid, "upd", ttb.fields(user + ".ttb_dmloai", "id=" + l_id));
            }
            if (!ttb.upd_dmloai(l_id, ten.Text, int.Parse(nhomcc.SelectedValue.ToString()), i_nhom, int.Parse(stt.Value.ToString())))
			//if (!ttb.upd_dmloai(l_id,ten.Text,0,i_nhom,int.Parse(stt.Value.ToString())))
			{
		...
		private void butHuy_Click(object sender, System.EventArgs e)
		{
			...
			try
			{
				if (ttb.get_data("select * from "+user+".ttb_dmbd where maloai="+decimal.Parse(dataGrid1[dataGrid1.CurrentCell.RowNumber,3].ToString())).Tables[0].Rows.Count!=0)
				{
			...
			if (MessageBox.Show("Đồng ý hủy thông tin này !",ttb.Msg,MessageBoxButtons.YesNo,MessageBoxIcon.Question)==DialogResult.Yes)
			{
                ...
                ttb.upd_eve_upd_del(itable, i_userid, "del", ttb.fields(user + ".ttb_dmloai", "id=" + l_id));
				ttb.execute_data("delete from "+user+".ttb_dmloai where id="+l_id);
				if (!ttb.bDanhmuc) ttb.writeXml("ttb_thongso","c01","1");
				load_grid();
			}
		}
