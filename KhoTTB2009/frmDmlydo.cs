...
namespace TTB
{
	...
	/// </summary>
	public class frmDmlydo : System.Windows.Forms.Form
	{
		...
		private int l_id,i_nhom,i_userid,itable;
        private string user;
		...
		{
            System.ComponentModel.ComponentResourceManager resources = new System.ComponentModel.ComponentResourceManager(typeof(frmDmlydo));
            ...
            this.Text = "Danh mục nhập xuất";
            this.WindowState = System.Windows.Forms.FormWindowState.Maximized;
            this.KeyDown += new System.Windows.Forms.KeyEventHandler(this.frmDmlydo_KeyDown);
            this.Load += new System.EventHandler(this.frmDmlydo_Load);
		...
		private void frmDmlydo_Load(object sender, System.EventArgs e)
		{
            if (Screen.PrimaryScreen.WorkingArea.Width > 800) this.WindowState = System.Windows.Forms.FormWindowState.Normal;
            user = ttb.user; itable = ttb.tableid("", "ttb_dmlydotscd");
			dsxml.ReadXml("..\\..\\..\\xml\\ttb_nhapxuat.xml");
			nhomcc.DisplayMember="TEN";
			nhomcc.ValueMember="ID";
			nhomcc.DataSource=dsxml.Tables[0];
			load_grid();
			AddGridTableStyle();
			ref_text();
		}
		...
		{
			dt=ttb.get_data("select a.*,case when a.loai=0 then 'Nhập' else 'Xuất' end as nhapxuat from "+user+".ttb_dmlydotscd a where a.nhom="+i_nhom+" order by a.loai,a.stt").Tables[0];
		...
		private void butMoi_Click(object sender, System.EventArgs e)
		{
			try
			{
                stt.Value = decimal.Parse(ttb.get_data("select max(stt) from " + user + ".ttb_dmlydotscd where nhom=" + i_nhom).Tables[0].Rows[0][0].ToString()) + 1;
			}
			catch{stt.Value=1;}
			l_id=0;
			ten.Text="";
			nhomcc.SelectedIndex=0;
			ena_object(true);
			stt.Focus();
		}
		...
		private void butSua_Click(object sender, System.EventArgs e)
		{
			...
			l_id=int.Parse(dataGrid1[dataGrid1.CurrentCell.RowNumber,3].ToString());
			ena_object(true);
			stt.Focus();
		}
		...
		private void butLuu_Click(object sender, System.EventArgs e)
		{
            ...
            if (l_id == 0)
            {
                try
                {
                    l_id = int.Parse(ttb.get_data("select max(id) from " + user + ".ttb_dmlydotscd").Tables[0].Rows[0][0].ToString()) + 1;
                }
                catch { l_id = 1; }
            ...
            else
            {
                ...
                ttb.upd_eve_upd_del(itable, i_userid, "upd", ttb.fields(user + ".ttb_dmlydotscd", "id=" + l_id));
            }
			if (!ttb.upd_dmlydotscd(l_id,ten.Text,int.Parse(stt.Value.ToString()),i_nhom,nhomcc.SelectedIndex))
			{
		...
		private void butHuy_Click(object sender, System.EventArgs e)
		{
			...
			if (MessageBox.Show("Đồng ý hủy thông tin này !",ttb.Msg,MessageBoxButtons.YesNo,MessageBoxIcon.Question)==DialogResult.Yes)
			{
                ...
                ttb.upd_eve_upd_del(itable, i_userid, "del", ttb.fields(user + ".ttb_dmlydotscd", "id=" + l_id));
				ttb.execute_data("delete from "+user+".ttb_dmlydotscd where id="+l_id);
				if (!ttb.bDanhmuc) ttb.writeXml("ttb_thongso","c01","1");
				load_grid();
			}
		}
		...
		private void ref_text()
		{
			try
			{
				...
				nhomcc.SelectedIndex=int.Parse(dataGrid1[i,4].ToString());
			}
			catch{}
		}
		...
		private void frmDmlydo_KeyDown(object sender, System.Windows.Forms.KeyEventArgs e)
		{
