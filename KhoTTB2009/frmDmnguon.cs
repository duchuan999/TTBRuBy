...
namespace TTB
{
	...
	public class frmDmnguon : System.Windows.Forms.Form
	{
		...
		private void InitializeComponent()
		{
            ...
            this.butBoqua.Size = new System.Drawing.Size(63, 25);
            ...
            this.butHuy.Size = new System.Drawing.Size(61, 25);
            ...
            // label3
            // 
            this.label3.Location = new System.Drawing.Point(-1, 461);
            ...
            // label4
            // 
            this.label4.Location = new System.Drawing.Point(400, 461);
            ...
            this.nhomcc.Location = new System.Drawing.Point(448, 461);
            this.nhomcc.Name = "nhomcc";
            this.nhomcc.Size = new System.Drawing.Size(176, 21);
            this.nhomcc.TabIndex = 2;
            this.nhomcc.KeyDown += new System.Windows.Forms.KeyEventHandler(this.nhomcc_KeyDown);
            // 
            // label1
            // 
            this.label1.Location = new System.Drawing.Point(619, 459);
            ...
            "Viện trợ",
            "Khác"});
            this.loaibc.Location = new System.Drawing.Point(664, 461);
            this.loaibc.Name = "loaibc";
            this.loaibc.Size = new System.Drawing.Size(120, 21);
		...
		private void frmDmnguon_Load(object sender, System.EventArgs e)
		{
            ...
            user = ttb.user; itable = ttb.tableid("", "ttb_dmnguon");
		...
		{
            dt = ttb.get_data("select a.*,b.ten as tennhom,case when a.loaibc=0 then 'Nhà nước' else case when a.loaibc=1 then 'Viện trợ' else 'Khác' end end as tenloai from " + user + ".ttb_dmnguon a left join " + user + ".ttb_nhomkho b on a.loai=b.id where a.nhom=" + i_nhom + " order by a.stt").Tables[0];
		...
		private void butMoi_Click(object sender, System.EventArgs e)
		{
			try
			{
                stt.Value = decimal.Parse(ttb.get_data("select max(stt) from " + user + ".ttb_dmnguon where nhom=" + i_nhom).Tables[0].Rows[0][0].ToString()) + 1;
			}
			catch{stt.Value=1;}
			l_id=0;
			ten.Text="";
		...
		private void butLuu_Click(object sender, System.EventArgs e)
		{
            ...
            if (l_id == 0)
            {
                try
                {
                    l_id = decimal.Parse(ttb.get_data("select max(id) from " + user + ".ttb_dmnguon").Tables[0].Rows[0][0].ToString()) + 1;
                }
                catch { l_id = 1; }
            ...
            else
            {
                ...
                ttb.upd_eve_upd_del(itable, i_userid, "upd", ttb.fields(user + ".ttb_dmnguon", "id=" + l_id));
            }
			if (!ttb.upd_dmnguon(l_id,ten.Text,i_nhom,int.Parse(nhomcc.SelectedValue.ToString()),int.Parse(stt.Value.ToString())))
			{
				MessageBox.Show("Không cập nhật thông tin "+this.Text.Trim()+" !",ttb.Msg);
				return;
			}
			ttb.execute_data("update "+user+".ttb_dmnguon set loaibc="+loaibc.SelectedIndex+" where id="+l_id);
			load_grid();
			ena_object(false);
		}
		...
		private void butHuy_Click(object sender, System.EventArgs e)
		{
			...
			try
			{
				if (ttb.get_data("select * from "+user+".ttb_tonkhoth where manguon="+decimal.Parse(dataGrid1[dataGrid1.CurrentCell.RowNumber,3].ToString())).Tables[0].Rows.Count!=0)
				{
			...
			if (MessageBox.Show("Đồng ý hủy thông tin này !",ttb.Msg,MessageBoxButtons.YesNo,MessageBoxIcon.Question)==DialogResult.Yes)
			{
                ...
                ttb.upd_eve_upd_del(itable, i_userid, "del", ttb.fields(user + ".ttb_dmnguon", "id=" + l_id));
				ttb.execute_data("delete from "+user+".ttb_dmnguon where id="+l_id);
				load_grid();
			}
		}
