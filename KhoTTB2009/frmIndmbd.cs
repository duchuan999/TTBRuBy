...
namespace TTB
{
	...
	public class frmIndmbd : System.Windows.Forms.Form
	{
		...
		public frmIndmbd(int nhom,string makho, string mmyy)
		{
			InitializeComponent();
			i_nhom=nhom;
		...
		private System.Data.DataTable get_dmbd()
		{
			string sql="select distinct a.ma, (a.ten||' '||a.hamluong) tenbd, a.tenhc, a.dang, c.losx, c.handung, d.ten nhacc, e.ten nuocsx, f.ten hangsx, decode(a.manuoc,0,1,e.loai) noingoai, g.ten nhom, a.manhom, a.manuoc, a.mahang "+
				" from ttb_dmbd a, ttb_nhapll b, ttb_nhapct c, ttb_dmnx d, ttb_dmnuoc e, ttb_dmhang f, ttb_dmnhom g "+
            ...
            System.Data.DataTable dt = ttb.get_data(sql).Tables[0];
			return dt;
		}
		...
		private void get_biendonggia()
		{
			...
			if(blnngay==true)
			{
				sql="select distinct b.mabd, a.ma, a.ten,a.hamluong, a.tenhc,a.dang, d.ten nuocsx, decode(a.manuoc,0,1,d.loai) noingoai, c.ten nhom, a.manhom, a.manuoc, b.dongia, b.ngayud, b.ngay, to_char(b.ngay,'ddmmyy') ng "+
					" from ttb_dmbd a, ttb_theodoigia b, ttb_dmnhom c, ttb_dmnuoc d "+
			...
			{
				DataRow r1=ttb.getrowbyid(ds.Tables[0],"mabd="+int.Parse(r["mabd"].ToString()));				
				if ( r1 == null )
				{					
		...
		private void get_gia()
		{
			...
			string sql="select distinct b.mabd, a.ma, a.ten,a.hamluong, a.tenhc,a.dang,e.dongia giatruoc,f.dongia giasau,f.dongia-e.dongia chenhlech"+
					" from ttb_dmbd a, ttb_theodoigia b,"+
					" (select mabd,dongia from ttb_theodoigia where to_char(ngay,'mmyy')='"+s_tu+"') e,"+
					" (select mabd,dongia from ttb_theodoigia where to_char(ngay,'mmyy')='"+s_den+"') f"+
		...
		private void frmIndmbd_Load(object sender, System.EventArgs e)
		{
            ...
            dtnhombd = ttb.get_data("select * from ttb_dmnhom").Tables[0];
		...
		private System.Data.DataTable Get_ngaydoi(string s_tu, string s_den, string s_nam, bool blnngay)
		{
			...
			string sql="";
			if(blnngay)
				sql="select distinct to_char(ngay,'dd/mm/yy') ngaydoi, to_char(ngay,'ddmmyy') ngay from ttb_theodoigia a, ttb_dmbd b where a.mabd=b.id and to_number(substr(to_char(ngay,'mmyy'),3,2)||substr(to_char(ngay,'mmyy'),1,2))>=to_number('"+s_tu+"') and to_number(substr(to_char(ngay,'mmyy'),3,2)||substr(to_char(ngay,'mmyy'),1,2))<=to_number('"+s_den+"')";
			else
				sql="select distinct to_char(ngay,'mm/yy') ngaydoi, to_char(ngay,'mmyy') ngay from ttb_theodoigia a, ttb_dmbd b where a.mabd=b.id and to_number(substr(to_char(ngay,'mmyy'),3,2)||substr(to_char(ngay,'mmyy'),1,2))>=to_number('"+s_tu+"') and to_number(substr(to_char(ngay,'mmyy'),3,2)||substr(to_char(ngay,'mmyy'),1,2))<=to_number('"+s_den+"')";
			if (s_nhombd!="") sql+=" and b.manhom in ("+s_nhombd.Substring(0,s_nhombd.Length-1)+")";
            ldt = ttb.get_data(sql).Tables[0];
			return ldt;
		}
		private void Tao_dataset()
		{
			ds=new DataSet();
			ds.ReadXml("..\\..\\..\\xml\\ttb_theodoigia.xml");
