...
namespace TTB
{
	...
	public class frmNhomctd : System.Windows.Forms.Form
	{
		...
		public frmNhomctd(AccessData acc,int nhom,int userid)
		{
			//
		...
		private void InitializeComponent()
		{
            ...
            // label4
            // 
            this.label4.Location = new System.Drawing.Point(674, 461);
            ...
            this.dvt.Location = new System.Drawing.Point(720, 461);
            this.dvt.Name = "dvt";
		...
		private void frmNhomctd_Load(object sender, System.EventArgs e)
		{
            ...
            user = ttb.user; itable = ttb.tableid("", "ttb_nhomctd");
			load_grid();
			AddGridTableStyle();
			ref_text();
		}
		...
		{
			dt=ttb.get_data("select * from "+user+".ttb_nhomctd where nhom="+i_nhom+" order by stt").Tables[0];
		...
		private void butMoi_Click(object sender, System.EventArgs e)
		{
			try
			{
				stt.Value=decimal.Parse(ttb.get_data("select max(stt) from "+user+".ttb_nhomctd where nhom="+i_nhom).Tables[0].Rows[0][0].ToString())+1;
			}
			catch{stt.Value=1;}
			l_id=0;
			ten.Text="";
		...
		private void butLuu_Click(object sender, System.EventArgs e)
		{
            if (l_id == 0)
            {
                try
                {
                    l_id = decimal.Parse(ttb.get_data("select max(id) from " + user + ".ttb_nhomctd").Tables[0].Rows[0][0].ToString()) + 1;
                }
                catch { l_id = 1; }
            ...
            else
            {
                ...
                ttb.upd_eve_upd_del(itable, i_userid, "upd", ttb.fields(user + ".ttb_nhomctd", "id=" + l_id));
            }
			if (!ttb.upd_nhomctd(l_id,ten.Text,dvt.Text,i_nhom,int.Parse(stt.Value.ToString())))
			{
		...
		private void butHuy_Click(object sender, System.EventArgs e)
		{
			...
			try
			{
				if (ttb.get_data("select * from "+user+".ttb_dmloai where loai="+decimal.Parse(dataGrid1[dataGrid1.CurrentCell.RowNumber,3].ToString())).Tables[0].Rows.Count!=0)
				{
			...
			if (MessageBox.Show("Đồng ý hủy thông tin này !",ttb.Msg,MessageBoxButtons.YesNo,MessageBoxIcon.Question)==DialogResult.Yes)
			{
                ...
                ttb.upd_eve_upd_del(itable, i_userid, "del", ttb.fields(user + ".ttb_nhomctd", "id=" + l_id));
				ttb.execute_data("delete from "+user+".ttb_nhomctd where id="+l_id);
				load_grid();
                dataGrid1_CurrentCellChanged(null, null);
			}
		}
