...
namespace TTB
{
	...
	public class frmThdutru : System.Windows.Forms.Form
	{
		...
		private void InitializeComponent()
		{
            ...
            this.butIn.Location = new System.Drawing.Point(32, 96);
		...
		{
            sql = "select a.*, b.stt as tt, b.ten as tennhom, a.maloai as idnn, f.ten as noingoai from " + user + ".ttb_dmbd a, " + user + ".ttb_dmnhom b, " + user + ".ttb_dmhang e, " + user + ".ttb_dmloai f";
		...
		private string get_ten(int i)
		{
			...
			string []map={"TT","Tên vật tư","ĐVT","Đơn giá",s,"Tổng số"};
			return map[i];
		}
		...
		private void tao_table()
		{
			...
			dtmakp.Clear();
			s_mmyy=tu.Value.ToString().PadLeft(2,'0')+yyyy.Value.ToString().PadLeft(4,'0').Substring(2,2);
			if (ttb.bMmyy(s_mmyy)) get_xuat_makp(s_mmyy);
			dc=new DataColumn();
		...
		{
            xxx = user;// +d_mmyy;
			sql="select distinct khon from "+xxx+".ttb_dutrucapll a,"+xxx+".ttb_dutrucapct b where a.id=b.id and a.nhom="+i_nhom;
            sql += " and a.mmyy='" + d_mmyy + "' and a.khox=" + int.Parse(makho.SelectedValue.ToString());
			if (manguon.SelectedIndex!=-1) sql+=" and b.manguon="+int.Parse(manguon.SelectedValue.ToString());
			foreach(DataRow r in ttb.get_data(sql).Tables[0].Rows) updrec(r["khon"].ToString().Trim());
		}
		...
		private void get_xuat(string d_mmyy)
		{
            ...
            sql += " from " + xxx + ".ttb_dutrucapll a," + xxx + ".ttb_dutrucapct b where a.mmyy='" + d_mmyy + "' and  a.id=b.id and a.khox=" + int.Parse(makho.SelectedValue.ToString()) + " and a.nhom=" + i_nhom;
			if (manguon.SelectedIndex!=-1) sql+=" and b.manguon="+int.Parse(manguon.SelectedValue.ToString());
			...
			sql="select a.mabd,sum(a.tondau+a.slnhap-a.slxuat) as toncuoi from "+xxx+".ttb_tonkhoct a,"+xxx+".ttb_theodoi t ";
			sql+=" where a.stt=t.id and a.makho="+int.Parse(makho.SelectedValue.ToString());
			sql+=" and a.tondau+a.slnhap-a.slxuat>0 and mmyy='"+d_mmyy+"'";
		...
		private void get_data()
		{
			tao_table();
			s_mmyy=tu.Value.ToString().PadLeft(2,'0')+yyyy.Value.ToString().PadLeft(4,'0').Substring(2,2);
			if (ttb.bMmyy(s_mmyy)) get_xuat(s_mmyy);
		...
		private void updrec (string makp)
		{
			...
			if ( r == null )
			{
				DataRow r1=ttb.getrowbyid(dtdmkho,"id="+int.Parse(makp));
				if (r1!=null)
				{
