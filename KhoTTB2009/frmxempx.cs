...
namespace Duoc
{
	...
	public class frmxempx : System.Windows.Forms.Form
	{
		...
		{
			System.Resources.ResourceManager resources = new System.Resources.ResourceManager(typeof(frmxempx));
			...
			this.butKetthuc.Location = new System.Drawing.Point(440, 488);
			...
			this.butIn.Location = new System.Drawing.Point(360, 488);
			...
			this.nhomin.Font = new System.Drawing.Font("Tahoma", 8.25F, System.Drawing.FontStyle.Regular, System.Drawing.GraphicsUnit.Point, ((System.Byte)(0)));
			...
			this.butRef.Location = new System.Drawing.Point(280, 488);
		...
		private void load_sophieuxuat(int i_makp, int i_nhomin)
		{
			...
			ds_phieu=d.get_data(s_mmyy,sql);
		...
		private void load_xuat_ct(int i_nhom, int i_loai,int i_phieu,int i_makp,int i_makho,string s_madt,string s_manguon, string s_ngay, int i_nhomin)
		{
			...
			/*
			string sql="select rownum as stt_t, b.makho,b.mabd,sum(b.soluong) soluong, c.ma, (c.ten||' '||c.hamluong) ten, c.tenhc, c.dang, d.ten tennguon, sum(b.soluong*b.giamua) sotien, round(b.sotien/b.soluong,3) dongia ";
			sql+=" from d_xuatsdll a,"+tenfile+" b, d_dmbd c, d_dmnguon d, d_dmnhom e ";
			sql+=" where a.id=b.id and b.mabd=c.id and b.manguon=d.id(+) and c.manhom=e.id and to_char(a.ngay,'dd/mm/yyyy')='"+s_ngay+"' and a.mmyy='"+s_mmyy+"'";
			sql+=" and a.nhom="+i_nhom+" and a.loai="+i_loai+" and a.phieu="+i_phieu+" and a.makp="+i_makp;
			if (s_madt!="" && tenfile=="d_thucxuat") sql+=" and b.madoituong in ("+s_madt.Substring(0,s_madt.Length-1)+")";
			sql+=" and b.makho="+i_makho+" and e.nhomin="+i_nhomin;
			if (s_manguon!="") sql+=" and b.manguon in ("+s_manguon.Substring(0,s_manguon.Length-1)+")";
			sql+=" group by b.makho,b.mabd, c.ma, c.ten, c.hamluong, c.tenhc, c.dang, d.ten,round(b.sotien/b.soluong,3) , rownum ";			
			dsct=d.get_data(sql);
			*/
			//
			string s_user=d.user;
			...
			if(d.bNoiNgoai(i_nhom))
			{
				sql+=" g.loai idnn, h.ten noingoai ";
				sql+=" from d_xuatsdll ll, "+tenfile+" a,"+s_user+".d_dmbd b,"+s_user+".d_dmnguon c,"+s_user+".d_nhomcc d,"+s_user+".d_dmkho f, "+s_user+".d_dmhang g, "+s_user+".d_nhomhang h, "+s_user+".d_dmnhom i, "+s_user+".d_dmnuoc j ";
				...
				sql+=" and b.manhom=i.id and b.mahang=g.id and g.loai=h.id and b.manuoc=j.id ";			
		...
		private void load_in_phieuxuat(decimal l_id)
		{
			...
			{
				sql+=" g.loai idnn, h.ten noingoai ";
				sql+=" from d_xuatsdct a,"+s_user+".d_dmbd b,"+s_user+".d_dmnguon c,"+s_user+".d_nhomcc d,"+s_user+".d_doituong e,"+s_user+".d_dmkho f, "+s_user+".d_dmhang g, "+s_user+".d_nhomhang h, "+s_user+".d_dmnhom i, "+s_user+".d_dmnuoc j ";
