...
namespace TTB
{
	...
	public class frmxkngay : System.Windows.Forms.Form
	{
		...
		private void InitializeComponent()
		{
            ...
            this.butIn.Location = new System.Drawing.Point(103, 200);
            ...
            this.butKetthuc.Location = new System.Drawing.Point(165, 200);
		...
		private void rptBcngay_Load(object sender, System.EventArgs e)
		{
            ...
            dt = ttb.get_data("select * from " + user + ".ttb_dmbd where nhom=" + i_nhom + " order by id").Tables[0];
		...
		private void Tao_dataset(DateTime d1,DateTime d2)
		{
			...
			dsngay.Clear();
			sql="select distinct to_char(ngay,'yymmdd') as ngay,to_char(ngay,'dd/mm') as ten from "+user+".ttb_xuatsdll where nhom="+i_nhom+" and ngay between to_date('"+tu.Text+"',"+stime+") and to_date('"+den.Text+"',"+stime+")";
            sql += "union all select distinct to_char(ngay,'yymmdd') as ngay,to_char(ngay,'dd/mm') as ten from "+user+".ttb_xuatll where nhom=" + i_nhom + " and loai='XK' and ngay between to_date('" + tu.Text + "'," + stime + ") and to_date('" + den.Text + "'," + stime + ")";
           // sql += "union all select distinct to_char(ngay,'yymmdd') as ngay,to_char(ngay,'dd/mm') as ten from xxx.ttb_ngtrull where nhom=" + i_nhom + " and ngay between to_date('" + tu.Text + "'," + stime + ") and to_date('" + den.Text + "'," + stime + ")";
           // sql += "union all select distinct to_char(a.ngay,'yymmdd') as ngay,to_char(a.ngay,'dd/mm') as ten from xxx.bhytkb a,xxx.bhytthuoc b where a.id=b.id and a.nhom=" + i_nhom + " and a.ngay between to_date('" + tu.Text + "'," + stime + ") and to_date('" + den.Text + "'," + stime + ")";
			dr=ttb.get_thuoc(tu.Text,den.Text,sql).Tables[0].Select("ngay<>''","ngay");
			for(int i=0;i<dr.Length;i++)
			{
				r1=ttb.getrowbyid(dsngay.Tables[0],"ngay='"+dr[i]["ngay"].ToString()+"'");
				if ( r1 == null )
				{
			...
			if (mmyy2!=mmyy1)
			{
				dr=ttb.get_data(sql).Tables[0].Select("ngay<>''","ngay");
				for(int i=0;i<dr.Length;i++)
				{
					r1=ttb.getrowbyid(dsngay.Tables[0],"ngay='"+dr[i]["ngay"].ToString()+"'");
					if ( r1 == null )
		...
		private void get_xuat(string s_mmyy)
		{
            xxx = user;// +s_mmyy;			
			//phieu linh+haophi
            sql = "select to_char(a.ngay,'yymmdd') as ngay,b.mabd,c.ten,sum(b.soluong) as soluong from " + xxx + ".ttb_xuatsdll a," + xxx + ".ttb_thucxuat b," + user + ".ttb_dmbd c, " + user + ".ttb_dmhang e," + xxx + ".ttb_theodoi t ";
			...
			//bu tu truc
            sql = "select to_char(a.ngay,'yymmdd') as ngay,b.mabd,c.ten,sum(b.soluong) as soluong from " + xxx + ".ttb_xuatsdll a," + xxx + ".ttb_thucbucstt b," + user + ".ttb_dmbd c, " + user + ".ttb_dmhang e," + xxx + ".ttb_theodoi t ";
			...
			ins_items();
			//d_xuatll+d_xuatct
            sql = "select to_char(a.ngay,'yymmdd') as ngay,b.mabd,c.ten,sum(b.soluong) as soluong from " + xxx + ".ttb_xuatll a," + xxx + ".ttb_xuatct b," + user + ".ttb_dmbd c, " + user + ".ttb_dmhang e," + xxx + ".ttb_theodoi t ";
			...
			ins_items();
			//d_ngtrull+d_ngtruct
            //sql = "select to_char(a.ngay,'yymmdd') as ngay,b.mabd,c.ten,sum(b.soluong) as soluong from " + xxx + ".ttb_ngtrull a," + xxx + ".ttb_ngtruct b," + user + ".ttb_dmbd c, " + user + ".ttb_dmhang e," + xxx + ".ttb_theodoi t ";
            ...
            //ins_items();
            ////bhytkb+bhytthuoc
            //sql = "select to_char(a.ngay,'yymmdd') as ngay,b.mabd,c.ten,sum(b.soluong) as soluong from " + xxx + ".bhytkb a," + xxx + ".bhytthuoc b," + user + ".ttb_dmbd c, " + user + ".ttb_dmhang e," + xxx + ".ttb_theodoi t ";
            //sql += "where a.id=b.id and b.sttt=t.id and b.mabd=c.id and c.mahang=e.id and a.nhom=" + i_nhom + " and a.ngay between to_date('" + tu.Text + "'," + stime + ") and to_date('" + den.Text + "'," + stime + ")";
            //if (s_kho!="") sql+=" and b.makho in ("+s_kho.Substring(0,s_kho.Length-1)+")";
            //if (manguon.SelectedIndex!=-1) sql+=" and t.manguon="+int.Parse(manguon.SelectedValue.ToString());
            //sql+=" group by a.ngay,b.mabd,c.ten";
