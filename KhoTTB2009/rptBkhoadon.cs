...
namespace TTB
{
	...
	public class rptBkhoadon : System.Windows.Forms.Form
	{
		...
		public rptBkhoadon(AccessData acc,int nhom,string kho)
		{
			//
		...
		private void InitializeComponent()
		{
            ...
            this.butIn.Location = new System.Drawing.Point(247, 236);
            ...
            this.butKetthuc.Location = new System.Drawing.Point(309, 236);
		...
		private void rptBkhoadon_Load(object sender, System.EventArgs e)
		{
			...
			if (ttb.bQuanlynguon(i_nhom)) sql+=" where nhom="+i_nhom;
			else sql+=" where id=0";
			sql+=" order by stt";
		...
		private void butIn_Click(object sender, System.EventArgs e)
		{
			...
			for(int i=y1;i<=y2;i++)
			{
				...
				for(int j=itu;j<=iden;j++)
				{
					...
					if (ttb.bMmyy(mmyy)) get_nhap(mmyy);
				}
			}
		...
		private void get_nhap(string d_mmyy)
		{
            xxx = user + d_mmyy;
			string ten="";
			sql="select c.id,b.stt,b.mabd,trim(d.ten)||' '||trim(d.hamluong)||' '||d.dang as tenbd,a.sohd,to_char(a.ngayhd,'dd/mm/yyyy') as ngayhd,b.sotien+b.sotien*b.vat/100+b.cuocvc+b.chaythu as sotien ";
            sql += " from " + xxx + ".ttb_nhapll a," + xxx + ".ttb_nhapct b," + user + ".ttb_dmnx c," + user + ".ttb_dmbd d";
			...
			{
				r1=ttb.getrowbyid(dt,"id="+int.Parse(r["id"].ToString()));
				if (r1!=null)
				{
		...
		private void butLoc_Click(object sender, System.EventArgs e)
		{
			...
			for(int i=y1;i<=y2;i++)
			{
				...
				for(int j=itu;j<=iden;j++)
				{
					...
					{
                        xxx = user + mmyy;
						sql="select distinct b.* from "+xxx+".ttb_nhapll a,"+user+".ttb_dmnx b where a.madv=b.id";
						sql+=" and a.ngayhd between to_date('"+tu.Text+"',"+stime+") and to_date('"+den.Text+"',"+stime+")";
						if (kho.SelectedIndex!=-1) sql+=" and a.makho="+int.Parse(kho.SelectedValue.ToString());
						...
						if (be) dsdmnx=ttb.get_data(sql);
						else dsdmnx.Merge(ttb.get_data(sql));
						be=false;
					}
				}
			}
