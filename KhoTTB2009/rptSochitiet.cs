...
namespace TTB
{
	...
	public class rptSochitiet : System.Windows.Forms.Form
	{
		...
		public rptSochitiet(AccessData acc,int nhom,string kho,string mmyy,bool giaban)
		{
			//
		...
		private void InitializeComponent()
		{
            ...
            this.butKetthuc.Location = new System.Drawing.Point(310, 378);
		...
		private void rptSochitiet_Load(object sender, System.EventArgs e)
		{
            ...
            sql = "select * from " + user + ".ttb_dmkho where nhom=" + i_nhom;
			sql+=" order by stt";
			tmpkho=ttb.get_data(sql).Tables[0];
            ...
            dtkp = ttb.get_data("select * from " + user + ".d_duockp where nhom like '%" + i_nhom.ToString() + ",%'" + " order by stt").Tables[0];
            dt = ttb.get_data("select a.*,b.ten as tenhang,c.ten as tennuoc,d.ten as tennhom from " + user + ".ttb_dmbd a," + user + ".ttb_dmhang b," + user + ".ttb_dmnuoc c," + user + ".ttb_dmnhom d where a.mahang=b.id and a.manuoc=c.id and a.manhom=d.id and a.nhom=" + i_nhom + " order by a.id").Tables[0];
		...
		private void load_grid()
		{
            ...
            xxx = user + s_mmyy;
            sql = "select a.oid as stt,a.mabd,b.ma,trim(b.ten)||' '||b.hamluong as ten,b.tenhc,b.dang,c.ten as tenhang,a.tondau+a.slnhap-a.slxuat as toncuoi from " + xxx + ".ttb_tonkhoth a," + user + ".ttb_dmbd b," + user + ".ttb_dmhang c where (a.mabd=b.id and b.mahang=c.id ";
			if (manguon.SelectedIndex!=-1) sql+=" and a.manguon="+int.Parse(manguon.SelectedValue.ToString());
			if (i_loai!=0) sql+=" and b.maloai<>"+i_loai;
			sql+=" and a.makho in (select id from "+user+".ttb_dmkho where ketoan=0 and nhom="+i_nhom+")) and (a.tondau<>0 or a.slnhap<>0 or a.slxuat<>0)";
			sql+=" union all ";
            sql += " select a.oid as stt,a.mabd,b.ma,trim(b.ten)||' '||b.hamluong ten,b.tenhc,b.dang,c.ten as tenhang,a.tondau+a.slnhap-a.slxuat as toncuoi from " + xxx + ".ttb_tutructh a," + user + ".ttb_dmbd b," + user + ".ttb_dmhang c where (a.mabd=b.id and b.mahang=c.id ";
			if (manguon.SelectedIndex!=-1) sql+=" and a.manguon="+int.Parse(manguon.SelectedValue.ToString());
			if (i_loai!=0) sql+=" and b.maloai<>"+i_loai;
			sql+=" and a.makho in (select id from "+user+".ttb_dmkho where ketoan=0 and nhom="+i_nhom+")) and (a.tondau<>0 or a.slnhap<>0 or a.slxuat<>0)";
			foreach(DataRow r in ttb.get_data(sql).Tables[0].Select("true","ten"))
				ttb.updrec_thekho(dsdm.Tables[0],int.Parse(r["mabd"].ToString()),r["ma"].ToString(),r["ten"].ToString(),r["tenhc"].ToString(),r["dang"].ToString(),r["tenhang"].ToString(),decimal.Parse(r["toncuoi"].ToString()),int.Parse(r["stt"].ToString()));
		...
		private void Loc()
		{
			...
			if (dr.Length==0) foreach(DataRow r in dsdm.Tables[0].Rows) ttb.execute_data("insert into "+user+".ttb_mabd(mabd) values ("+int.Parse(r["mabd"].ToString())+")");
			else for(int i=0;i<dr.Length;i++) ttb.execute_data("insert into "+user+".ttb_mabd(mabd) values ("+int.Parse(dr[i]["mabd"].ToString())+")");
		}
		...
		private void get_sort()
		{
			...
			string s_ma="";
			dr=ds.Tables[0].Select("ma<>''","ten,yymmdd");
			for(int i=0;i<dr.Length;i++)
			{
				...
				dsxml.Merge(ttb.ins_items(dsxml,dr,i,slton,stton));
			}
		}
