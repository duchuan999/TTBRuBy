...
namespace TTB
{
	...
	public class rptThpnhap : System.Windows.Forms.Form
	{
		...
		private List listNX;
		private Medisoft2009.Usercontrol.MaskedTextBox diachi;
		private Medisoft2009.Usercontrol.MaskedTextBox masothue;
		private Medisoft2009.Usercontrol.MaskedTextBox sotk;
		...
		private void rptThpnhap_Load(object sender, System.EventArgs e)
		{
			...
			dtdmnx=ttb.get_data("select ma,ten,id,nhomcc,sotk,masothue,diachi from "+user+".ttb_dmnx where nhom="+i_nhom+" order by stt").Tables[0];
			...
			dtnhom=ttb.get_data("select * from "+user+".ttb_nhombc where nhom="+i_nhom+" order by id").Tables[0];
			ds.ReadXml("..\\..\\..\\xml\\ttb_Thpnhap.xml");
		}
		...
		{
            xxx = user;// +mmyy;
			sql="select d.loai as manhom,round(b.sotien+b.sotien*b.vat/100,3) as sotien from "+xxx+".ttb_nhapll a,"+xxx+".ttb_nhapct b,"+user+".ttb_dmbd c,"+user+".ttb_dmnhom d where a.id=b.id and b.mabd=c.id and c.manhom=d.id and a.nhom="+i_nhom+" and a.ngaysp between to_date('"+tu.Text+"',"+stime+") and to_date('"+den.Text+"',"+stime+")";
			...
			{
				r1=ttb.getrowbyid(ds.Tables[0],"manhom="+int.Parse(r["manhom"].ToString()));
				if (r1==null)
				{
					r2=ttb.getrowbyid(dtnhom,"id="+int.Parse(r["manhom"].ToString()));
