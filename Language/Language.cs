			...
			foreach(Control ctl in frm.Controls)
			{
				switch (ctl.GetType().ToString())
				{
					...
					case "System.Windows.Forms.Button":
						...
						string[] arr = lbl.Split(' ');
						ctl.Text=((arr.Length>0)?rong.PadLeft(7,' '):"")+get_text(ds.Tables[0],"vviet='"+lbl.Trim()+"'").Trim();
						break;
						...
						foreach(TabPage tab in tabclt.TabPages )
						{
							foreach(Control ct in tab.Controls )
							{
								switch(ct.GetType().ToString())
								{
									...
									case "System.Windows.Forms.Button":
										lbl=ct.Text.TrimEnd();
										string[] arr3 = lbl.Split(' ');
										ct.Text=((arr3.Length>0)?rong.PadLeft(7,' '):"")+get_text(ds.Tables[0],"vviet='"+lbl.Trim()+"'").Trim();
										break;
									default:
										lbl=ct.Text.Trim();
										if (lbl!="")
